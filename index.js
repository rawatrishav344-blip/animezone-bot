...
const isMovie = /#movie/i.test(trimmedCaption);
...
if (isAnimeInfo) {
  ...
  await updateAnimeByMessageId(messageId, mediaGroupId, messages, lines, isMovie);
...
async function handleIncomingPost(messages, caption) {
  ...
  const isMovie = /#movie/i.test(trimmed);
  ...
  if (isAnimeInfo) {
    await addAnimeInfo(messages, lines, isMovie);
  } else {
...
async function addAnimeInfo(messages, lines, isMovie = false) {
...
const alreadyExists = currentAnime.episodes.find(ep =>
  ...
  const epId = "e" + String(currentAnime.episodes.length + 1).padStart(3, "0");
    currentAnime.episodes.push({
      id: epId,
      title: currentAnime.isMovie ? "Movie" : title,
      episodeNumber,
      description,
...
async function updateAnimeByMessageId(messageId, mediaGroupId, messages, lines, isMovie = false) {
