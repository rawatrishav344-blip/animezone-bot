...
async function getThumbnailUrl(firstMsg) {
  ...
  try {
    ...
    const response = await axios.get(tgUrl, { responseType: 'stream', timeout: 15000 });
    const form = new FormData();
    form.append('file', response.data);

    // Graph.org ki jagah Telegra.ph use kar rahe hain
    ...
    const permanentLink = `https://telegra.ph${upload.data[0].src}`;
    console.log("✅ SUCCESS: Permanent Link ->", permanentLink);
    return permanentLink;
  } catch (err) {
    console.log("⚠️ Permanent upload failed, using temporary Telegram link.");
