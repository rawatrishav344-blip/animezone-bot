...
bot.on(["channel_post"], async (ctx) => {
  ...
  if (post.media_group_id) {
    ...
    pendingGroups[key].timer = setTimeout(() => {
      handleIncomingPost(pendingGroups[key].messages, pendingGroups[key].caption);
...
async function addEpisode(messages, lines) {
  ...
  const durationMatch = descLines.find((l) => /^\d+[hm]$/i.test(l.trim()));
...
function buildDeeplink(deliveryBotIndex, contentId) {
  const data = db.read();
...
app.get("/api/anime-by-category/:tag", (req, res) => {
  const data = db.read();
...
app.get("/api/anime/:id", (req, res) => {
  ...
  const episodes = sortedEpisodes.map((ep) => ({
    ...ep,
    deeplink: buildDeeplink(ep.deliveryBotIndex, `ep_${anime.id}_${ep.id}`),
  }));
...
app.get("/api/anime/:id/similar", (req, res) => {
  const data = db.read();
  const anime = data.animeList.find((a) => a.id === req.params.id);
  if (!anime) return res.status(404).json({ error: "Not found" });
...
app.post("/api/verify-dark-code", (req, res) => {
